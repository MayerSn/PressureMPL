...
void config_I2C(){
    ...
    DMA_InitStructure.DMA_BufferSize =3;	//number of data to be transfered
...
uint8_t I2C_DMA_Read(uint8_t slaveAddr){
    uint8_t state=0;
     ...
     //Set register adress for pressure sensor

      uint8_t  data_MSB_reg_addr=0x01;
      ...
      I2C_SendData(I2C1, data_MSB_reg_addr);
      j=0;
...
/**
  ...
  */
uint8_t I2C_Write(uint8_t slaveAddr, uint8_t regAddr, uint8_t numOfBytes, uint8_t maxSize, uint8_t *writeData){
        uint8_t state=0;
                int i=0,j=0;
...
void DMA1_Stream5_IRQHandler(void)
{
    ...
    DMA_SetCurrDataCounter(DMA_STREAM,3);
}
